/*
 * Heimdall
 *
 * # Introduction    The Heimdall backchannel API is a [RESTful](https://wikipedia.org/wiki/Representational_state_transfer) API that allows you to do authorizations, manage sessions, do CRUD on various objects, etc.          
 *
 * The version of the OpenAPI document: 1.0.0
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = heimdall_client.Client.OpenAPIDateConverter;

namespace heimdall_client.Model
{
    /// <summary>
    /// AuthorizeAcceptRequest
    /// </summary>
    [DataContract(Name = "AuthorizeAcceptRequest")]
    public partial class AuthorizeAcceptRequest : IEquatable<AuthorizeAcceptRequest>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="AuthorizeAcceptRequest" /> class.
        /// </summary>
        [JsonConstructorAttribute]
        protected AuthorizeAcceptRequest() { }
        /// <summary>
        /// Initializes a new instance of the <see cref="AuthorizeAcceptRequest" /> class.
        /// </summary>
        /// <param name="authSid">Authorization session identifier of the ongoing authorization session. (required).</param>
        /// <param name="sub">The identifier for the end-user (sub) (required).</param>
        public AuthorizeAcceptRequest(string authSid = default(string), string sub = default(string))
        {
            // to ensure "authSid" is required (not null)
            if (authSid == null) {
                throw new ArgumentNullException("authSid is a required property for AuthorizeAcceptRequest and cannot be null");
            }
            this.AuthSid = authSid;
            // to ensure "sub" is required (not null)
            if (sub == null) {
                throw new ArgumentNullException("sub is a required property for AuthorizeAcceptRequest and cannot be null");
            }
            this.Sub = sub;
        }

        /// <summary>
        /// Authorization session identifier of the ongoing authorization session.
        /// </summary>
        /// <value>Authorization session identifier of the ongoing authorization session.</value>
        [DataMember(Name = "auth_sid", IsRequired = true, EmitDefaultValue = false)]
        public string AuthSid { get; set; }

        /// <summary>
        /// The identifier for the end-user (sub)
        /// </summary>
        /// <value>The identifier for the end-user (sub)</value>
        [DataMember(Name = "sub", IsRequired = true, EmitDefaultValue = false)]
        public string Sub { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class AuthorizeAcceptRequest {\n");
            sb.Append("  AuthSid: ").Append(AuthSid).Append("\n");
            sb.Append("  Sub: ").Append(Sub).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as AuthorizeAcceptRequest);
        }

        /// <summary>
        /// Returns true if AuthorizeAcceptRequest instances are equal
        /// </summary>
        /// <param name="input">Instance of AuthorizeAcceptRequest to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(AuthorizeAcceptRequest input)
        {
            if (input == null)
            {
                return false;
            }
            return 
                (
                    this.AuthSid == input.AuthSid ||
                    (this.AuthSid != null &&
                    this.AuthSid.Equals(input.AuthSid))
                ) && 
                (
                    this.Sub == input.Sub ||
                    (this.Sub != null &&
                    this.Sub.Equals(input.Sub))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.AuthSid != null)
                {
                    hashCode = (hashCode * 59) + this.AuthSid.GetHashCode();
                }
                if (this.Sub != null)
                {
                    hashCode = (hashCode * 59) + this.Sub.GetHashCode();
                }
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        public IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
